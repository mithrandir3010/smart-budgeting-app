...
export default function BudgetLimitModal({ onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.status !== 409) {
        toast.error('Limit kaydedilirken hata oluştu.');
      }
      // 409 already handled by axios interceptor
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      {/* Modal */}
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-100 dark:border-zinc-800 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    setFormError('');
                    setForm((p) => ({ ...p, limitAmount: e.target.value }));
                  }}
